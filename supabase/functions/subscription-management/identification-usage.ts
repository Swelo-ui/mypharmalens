...
serve(async (req) => {
  ...
  try {
    ...
    const { data: dailyFree, error: dailyError } = await supabaseAdmin
      ...
      .select("remaining_identifications, last_claimed_at")
      .eq("user_id", userId)
    ...
    const lastClaimed = dailyFree?.last_claimed_at || null;
    ...
    const percentage = totalLimit > 0 ? Math.round((used / totalLimit) * 100) : 0;
    ...
    console.error("Error in identification-usage:", error);
    return new Response(
