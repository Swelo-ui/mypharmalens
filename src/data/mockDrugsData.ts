...
const detailedDrugData: Record<string, DetailedDrugData> = {
  '1': {
    id: '1',
    name: 'Lisinopril',
    genericName: 'Lisinopril',
...
export const getDetailedDrugData = (id: string): DetailedDrugData | null => {
  ...
  if (detailedDrugData[id]) {
    return detailedDrugData[id];
  }
  
  // If not found in detailed data, try to find in mockDrugsData and create a basic DetailedDrugData object
  const basicDrugData = combinedDrugsData.find(drug => drug.id === id);
